const ControllerSearchCarreer = require("../controllers/ControllerSearchCarreer");
const validateSession = require("../middlewares/validateSession");
/**
 * 
 * @param {Express.Application} app 
 */
const searchcarreer = (app) => {
    let response = {
        "data": "message",
        "code": "code"
    };

    app.route("/searchcarreer")
        .get(async (req, res) => {
            if (!(await validateSession(req, res, response))) return;
            if (req.query.name === "search") {
                const controller = new ControllerSearchCarreer();
                response.code = "200";
